...
def generate_with_groq(prompt, query):
    """Generate using Groq API - PRIMARY provider"""
    if not GROQ_API_KEY:
...
def generate_with_gemini(prompt, query):
    """Generate using Gemini API - FALLBACK provider"""
...
def generate_questions_with_answers(prompt, question_type, num_questions):
    """Generate questions WITH answer keys - Groq first, Gemini fallback"""
...
- Generate EXACTLY {num_questions} questions"""
    ...
    print("Attempting generation with Groq (primary)...")
    result = generate_with_groq(system_instruction, query)
    if result:
        result = re.sub(r'^(Here are|Here\'s|Below are|Sure).*?[:.\n]', '', result, flags=re.IGNORECASE | re.MULTILINE)
        result = re.sub(r'^#.*$', '', result, flags=re.MULTILINE)
        return result.strip()

    # ✅ Fallback to Gemini
    print("Groq failed, attempting Gemini (fallback)...")
    result = generate_with_gemini(system_instruction, query)
    if result:
        result = re.sub(r'^(Here are|Here\'s|Below are|Sure).*?[:.\n]', '', result, flags=re.IGNORECASE | re.MULTILINE)
        result = re.sub(r'^#.*$', '', result, flags=re.MULTILINE)
        return result.strip()
...
def save_questions_to_pdf(raw_questions, subject, marks, include_answers=True):
    ...
    while i < len(parts):
        if re.match(r'\d+\.', parts[i]):
            ...
            answer_patterns = [r'\*\*Answer:.*', r'\*\*Solution:.*', r'\*\*Model Answer:.*', r'\*\*Answer Guidelines:.*', r'Answer:.*$', r'Solution:.*$']
    ...
    for q in questions_only:
        clean_q = q
        for pat in [r'\*\*Answer:.*', r'\*\*Solution:.*', r'\*\*Model Answer:.*', r'\*\*Answer Guidelines:.*']:
            clean_q = re.sub(pat, '', clean_q, flags=re.IGNORECASE)
...
def create_google_form(questions, user_email):
    if not GOOGLE_FORMS_ENABLED:
        return None
    try:
        new_form = {"info": {"title": "Generated Assessment Form", "documentTitle": "Assessment Form"}}
    ...
    for line in lines:
        ...
        if re.match(r'^\d+\.', line):
            if current_question:
                ...
                questions.append({'question': current_question, 'type': q_type, 'options': current_options if current_options else None})
...
def add_question_to_form(service, form_id, question_text, question_type="text", choices=None):
    ...
    if question_type == "mcq" and choices:
        question = {"requests": [{"createItem": {"item": {"title": question_text, "questionItem": {"question": {"required": True, "choiceQuestion": {"type": "RADIO", "options": [{"value": choice} for choice in choices]}}}}, "location": {"index": question_count}}}]}
    else:
        question = {"requests": [{"createItem": {"item": {"title": question_text, "questionItem": {"question": {"required": True, "textQuestion": {"paragraph": True}}}}, "location": {"index": question_count}}}]}
    try:
    ...
    try:
        ...
        try:
            ...
            if output_format == 'pdf':
                ...
                return send_file(docx_output, download_name=fname, as_attachment=True, mimetype='application/vnd.openxmlformats-officedocument.wordprocessingml.document')
                ...
                try:
                    ...
                    if form_url:
                        ...
                        return jsonify({'success': False, 'message': 'Error creating Google Form.'}), 500
                except Exception as e:
